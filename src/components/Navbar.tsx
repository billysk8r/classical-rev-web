...
export default async function Navbar() {
  ...
  return (
    <nav className="flex flex-col lg:flex-row justify-between items-center p-6 bg-white dark:bg-black border-b border-zinc-200 dark:border-zinc-800 sticky top-0 z-50 gap-4 transition-colors">
      <div className="text-xl font-bold tracking-tighter uppercase italic text-zinc-900 dark:text-white shrink-0">
      ...
      <div className="flex flex-col md:flex-row items-center gap-6">
        ...
        {/* Pass the session to the button as a prop */}
        <LoginButton initialSession={session} />
      </div>
    </nav>
  );
}